...
"""
Process GEMMA LMM Results for Climate GWAS

This script processes the output files (.assoc.txt) from GEMMA LMM analysis for each
bioclimatic variable. It identifies statistically significant SNPs, maps them to
...
- GEMMA output files: /lmm_gemma/{biovar}/output/{biovar}.assoc.txt
...
for biovar in biovars:
    ...
    assoc[['rs', 'p_wald', 'beta', 'significant', 'blocks', 'af']].to_csv(
        path + f'/lmm_gemma/{biovar}/output/results_lmm.csv', 
        index=None
    )
